...
- [ ] 3.3 Ao arquivar esta change junto com `add-food-catalog`, reconciliar o tipo de quantidade do item alimentar em `diet-prescription` ("unidade" ou "porção em gramas") com a porção de referência de `food-catalog` (unidade livre), decidindo se `food-catalog` deve restringir sua porção de referência às mesmas duas opções para garantir correspondência no cálculo do total nutricional da receita
- [ ] 3.4 Registrar que os requisitos de login via Google, campo empresa opcional, política mínima de senha e consulta do próprio perfil em `nutritionist-auth` foram incorporados a partir de uma revisão de consistência com a especificação de implementação do backend (`nutri-back`, change `add-nutritionist-auth`, já implementada), e verificar periodicamente se novas specs de implementação do backend introduzem outras decisões de negócio ainda não refletidas neste System Spec
- [x] 3.5 Registrar que a rejeição de sessão automática no cadastro, a rejeição de nome/empresa só com espaços em branco e a rejeição de token Google inválido em `nutritionist-auth` foram incorporadas a partir de uma revisão de consistência com a especificação de implementação do frontend (`nutri-front`, change `add-nutritionist-auth`, já implementada), e verificar periodicamente se novas specs de implementação do frontend introduzem outras decisões de negócio ainda não refletidas neste System Spec
